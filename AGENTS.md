...
- **Mac mini = PRIMARY HOST** (since 2026-06-04, per ADR 011). Runs the full stack under **OrbStack + plain docker-compose** (no Coolify locally — it isn't native to macOS): `nexus-app` (Next.js), `claude-gateway`, `codex-gateway`, `nexus-sandbox`, `cloudflared` (the dedicated `nexus-mac` tunnel), and `cron-runner` (supercronic — replaces cron-job.org). Single compose + ops guide: [`services/local-os/`](services/local-os/README.md). One secret on the box (`DOPPLER_TOKEN` in gitignored `services/local-os/.env`); every container self-fetches from Doppler `prd`. Autostart via OrbStack login-item + the `com.nexus.local-os` LaunchAgent. Public hostnames `nexus` / `claude-gw` / `codex-gw` CNAME to the `nexus-mac` tunnel (`b741e21c…`) — but the tunnel is **opt-in (default OFF)**: `cloudflared` is in the compose `remote-access` profile so boot/`startup.sh` skips it; toggle public access for travel with `npm run access on|off|status` ([`services/local-os/nexus-access.sh`](services/local-os/nexus-access.sh)). Working locally on the Mac uses `127.0.0.1` with zero public ingress. **Per-business gateway containers** (`nexus-business-<slug>`) also run here (migrated off Coolify 2026-06-04) as a sibling compose project — managed host-side via `npm run business:local`, reachable by `nexus-app` over the shared `coolify` network at internal DNS `http://nexus-business-<slug>:3000` (no public tunnel needed since the app is local). Guide: [`services/local-os/businesses/`](services/local-os/businesses/README.md). Set `BUSINESS_RUNTIME=local` so the idle scale-down cron no-ops (always-on box; idle containers are ~free). **As of 2026-06-06 (P0 security migration, [PR #503](https://github.com/pinnacleadvisors/nexus/pull/503)) the entire stack AND Nexus development run under a dedicated STANDARD, non-admin macOS user `nexus-host` (uid 502) with its own per-user OrbStack — NOT the operator's iCloud-signed-in admin user `dylan_mini` (uid 501).** This isolates a gateway/claudecodeui RCE (or `privileged` sandbox escape) from the operator's live iCloud session — a container→macOS escape now lands as `nexus-host` (non-admin, no iCloud), bounding the blast radius to a throwaway user. Keep `nexus-host` standard, not admin. Ref [`docs/runbooks/p0-security-remediation.md`](docs/runbooks/p0-security-remediation.md). **Paperclip** (the `/workforce` engine — `npx paperclipai`, embedded-PostgreSQL, on `127.0.0.1:3100`) also runs under `nexus-host` via the `com.workforce.paperclip` LaunchAgent (RunAtLoad + KeepAlive) — migrated off `dylan_mini` 2026-06-09 (the last service moved, completing the isolation). `nexus-app` reaches it from its container at `host.docker.internal:3100` (`PAPERCLIP_API_BASE` default, unchanged); cutover runbook [`docs/runbooks/paperclip-cutover.md`](docs/runbooks/paperclip-cutover.md).
...
**Reusable loop templates live in [`.claude/loops/`](.claude/loops/README.md) — the loop-template library** (the file-based catalogue companion to the runtime `loops` table). Each template is a ready-to-instantiate `LoopCreateInput` + the concrete per-iteration steps. Before improvising a verify/iterate flow, check the library for a matching shape. Current templates: [`dev-prod-test-iterate`](.claude/loops/dev-prod-test-iterate.md) (change → dev test `npm run dev` + Playwright → if green, deploy + prod test → iterate until production-ready, cleaning up stray dev servers / browser tabs each cycle). `loop-runner` reads the matching template for its per-iteration steps; engineering / dev / codex / bug-hunt agents should follow `dev-prod-test-iterate` rather than hand-rolling a one-off verify. Reading a template never auto-runs it — instantiating a Loop stays operator-gated (the `loops` table is the only thing that spawns one).
