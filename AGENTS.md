...
| **`h5` — procedural skill** | Reusable named procedures the agent can invoke. Each is a small, evaluable unit. | [`.claude/skills/<name>/SKILL.md`](.claude/skills/) (firecrawl, frontend-design, molecularmemory, signals-briefing), [`skill-trainer`](.claude/agents/skill-trainer.md) (closed upskilling loop), [`/api/skills/[slug]/promote`](app/api/skills/[slug]/promote/route.ts) (draft → verified gate). |
...
- [ ] `npm run check:admin-trigger` passes — every route under `app/api/admin-trigger/*` has an `ALLOWED_USER_IDS` owner gate (v13). Routes there are owner-only manual triggers (NOT called by cron-job.org); forgetting the gate would expose a debug/rebuild action to any Clerk session. Use `// admin-check: ignore` only if the route is invoked by a trusted internal service (none today).
