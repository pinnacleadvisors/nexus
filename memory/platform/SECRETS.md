...
> 2026-06-04). `PAPERCLIP_API_BASE` (optional; default `http://host.docker.internal:3100`) — the Nexus
> `/workforce` page + `/api/workforce/*` proxy reach the local Paperclip orchestration server through it
> (Clerk-authed; Paperclip stays loopback-only). Gateway URL vars below resolve to the Mac now (KVM4 fallback until 2026-06-28). See
...
| `OPENROUTER_API_KEY` | **OpenRouter unified router.** Set `LLM_PROVIDER=openrouter` in Doppler to route the chat fallback path (`lib/llm/provider.ts`) through OpenRouter instead of direct Anthropic. Unifies access to GPT-5, Claude, Gemini, Llama, etc. behind one OpenAI-compatible API — useful when Claude Max + ChatGPT Pro subscriptions are exhausted or when the operator wants per-task model choice. Sign up at https://openrouter.ai. See `lib/llm/providers/openrouter.ts` for activation. |
| `OPENROUTER_DEFAULT_MODEL` | Optional. Overrides the built-in OpenRouter default (`anthropic/claude-sonnet-4-6`). Use OpenRouter's `<vendor>/<model>` slug format — e.g. `openai/gpt-5`, `google/gemini-2.5-pro`, `meta-llama/llama-3.3-70b`. Full catalog: https://openrouter.ai/models. |
| `OPENROUTER_BASE_URL` | Optional. Defaults to `https://openrouter.ai/api/v1`. Only override if proxying or testing against a mock. |
| `OPENROUTER_APP_URL` | Optional. Sets the `HTTP-Referer` header OpenRouter uses for app attribution. Falls back to `NEXT_PUBLIC_APP_URL` then unset. |
| `OPENROUTER_APP_TITLE` | Optional. Sets the `X-Title` header for app attribution. Defaults to `Nexus`. |
...
| `PLATFORM_CHAT_STREAM_ENABLED` | **SSE streaming for /manage-platform/chat.** Default `'1'` (on). Server-side gate on `/api/platform-chat/stream` — when `'0'`, the route returns HTTP 503 + `code: 'streaming_disabled'` and the client falls back to the existing async-poll path. Flip to `'0'` in Doppler for an instant rollback without redeploying. Audit follow-through, 2026-05-17. |
| `NEXT_PUBLIC_PLATFORM_CHAT_STREAM_ENABLED` | **Client mirror of the platform SSE flag.** Default `'0'` initially in production until smoke confirms the route works on Vercel preview, then flipped to `'1'`. When `'0'`, the client never attempts the stream POST → straight to existing poll (avoids the round-trip cost during rollback). Must be set client-side because Next.js inlines `NEXT_PUBLIC_*` vars at build time; needs a redeploy to flip. |
| `BUSINESS_CHAT_STREAM_ENABLED` | **SSE streaming for /businesses/[slug]/chat (Phase 2).** Default `'1'` (on). Server-side gate on `/api/businesses/[slug]/chat/stream` — same shape and rollback semantics as the platform flag. Independent so platform + business surfaces can roll out separately. Audit follow-through, 2026-05-17. |
| `NEXT_PUBLIC_BUSINESS_CHAT_STREAM_ENABLED` | **Client mirror of the business SSE flag.** Default `'0'` initially. Same shape as the platform mirror. Flip to `'1'` after smoke confirms the business stream route works on a preview deployment that has a per-business container provisioned. |
| `NEXUS_MEMORY_QUERY_URL` | **Optional.** When set, `lib/agents/inject-platform-brief.ts` POSTs to this URL during `/api/claude-session/dispatch` to fetch the top memory-hq atoms matching the spawn task. Typical value: `https://nexus.example.com/api/memory/query`. Skipped when unset — the brief still loads memory/INDEX.md + STACK.md + the most-relevant `task_plan-*.md` without the atom enrichment. Fail-soft: a 3s timeout protects the dispatch path from a slow query endpoint. |
...
| `COMPOSIO_API_KEY` | **New (Phase MCP)** — written into `~/.claude/settings.json` at boot. When set together with `SUPABASE_SERVICE_ROLE_KEY`, the entrypoint builds `services/mcp-composio-admin/` from `/repo` and registers it (hard-isolation: admin-scope only). When only `COMPOSIO_API_KEY` is set, falls back to `@composio/rube-mcp` (soft-isolation). When unset, no Composio MCP is registered. See PR #151 (admin-scope) + the wrapper PR. |
| `SUPABASE_SERVICE_ROLE_KEY` | **New (Phase MCP)** — read by the `mcp-composio-admin` wrapper to query `connected_accounts WHERE business_slug='_admin'`. Without this, the gateway can't build the wrapper and falls back to soft-isolation rube-mcp. Same key as Vercel-side `SUPABASE_SERVICE_ROLE_KEY`. |
