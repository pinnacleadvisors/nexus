...
export interface OperatorTaskRow {
  ...
  /** Cached step-by-step guide generated by the Explain button (migration 059). */
  explanation:              string | null
  /** ISO timestamp when `explanation` was last filled, for staleness checks. */
...
/**
 * Fetch a single task by id, scoped to the authenticated user_id. Returns
 * null when the row doesn't exist OR belongs to another user (RLS would
 * also block this server-side, but the explicit `eq('user_id', …)` keeps
 * the intent obvious to readers).
 */
export async function getTaskById(userId: string, id: string): Promise<OperatorTaskRow | null> {
  const db = createServerClient()
  if (!db) return null
  const res = await (db.from('operator_tasks' as never) as unknown as {
    select: (c: string) => { eq: (c: string, v: string) => { eq: (c: string, v: string) => { single: () => Promise<{ data: OperatorTaskRow | null }> } } }
  }).select('*').eq('id', id).eq('user_id', userId).single()
  return res.data ?? null
}

/**
 * Store the in-depth Explain guide on the row + stamp `explanation_generated_at`.
 * Called by /api/views/tasks/:id/explain after the LLM returns. The cache means
 * repeat clicks read from Postgres (free + instant) instead of the LLM.
 */
export async function setTaskExplanation(userId: string, id: string, explanation: string): Promise<OperatorTaskRow | null> {
  const db = createServerClient()
  if (!db) return null
  const updates = {
    explanation:              explanation.trim().slice(0, 12_000),
