...
// Headline guarantee: letters (CJK / accented) + math/currency symbols survive
// stripDecorative; only emoji/decorative are removed.
const intl = compress('日本語 café résumé €£¥ 5 < 10 and x > y ✅🚀')
check('CJK + accented letters preserved', /日本語/.test(intl.text) && /café/.test(intl.text) && /résumé/.test(intl.text))
check('currency + math symbols preserved', /€£¥/.test(intl.text) && intl.text.includes('5 < 10') && intl.text.includes('x > y'))
check('emoji stripped', !/[\u{1F000}-\u{1FAFF}]/u.test(intl.text))

// shortenUrls keeps a valid absolute URL (scheme preserved) when truncating.
const longUrl = compress('see https://shop.acme.com/products/category/very/long/path?a=1&b=2 now')
check('shortened URL keeps https:// scheme', /https:\/\/shop\.acme\.com/.test(longUrl.text))
