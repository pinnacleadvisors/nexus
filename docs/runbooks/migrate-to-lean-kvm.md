...
| Same-named app in multiple projects (script warns: `N apps named '<svc>' exist across projects`) | An earlier run created a duplicate under the wrong `project_uuid` (e.g. into `COOLIFY_PROJECT_ID_NEXUS_BUSINESSES` before the Platform project was wired). The script now soft-warns + operates on the first match instead of blocking | Delete the misplaced duplicates via `curl -X DELETE "$COOLIFY_KVM4_URL/api/v1/applications/<extra-uuid>" -H "Authorization: Bearer $COOLIFY_KVM4_API_TOKEN"`. The script logs each extra uuid so you know which to delete. Always confirm `COOLIFY_PROJECT_ID_NEXUS_PLATFORM` is set correctly in Doppler before `--apply`. |
