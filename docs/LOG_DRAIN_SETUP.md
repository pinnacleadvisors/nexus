...
| Drain active but no rows in `log_events` | Receiver returning 401 (`bad_signature`) | The custom secret in the wizard ≠ `VERCEL_LOG_DRAIN_SECRET` in Doppler. Double-check both. Vercel signs each batch with `x-vercel-signature: sha256(body)` using the value you typed in the wizard. |
