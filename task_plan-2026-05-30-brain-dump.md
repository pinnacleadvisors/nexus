...
- [ ] **Durable Chat (sub-plan 3)** — NOT shipped. Honest reason: every part resists dev+playwright verification — Phases A/B/C (tab-close survival) need a live gateway turn; Phase D (error persistence) needs failure-injection into the hot chat path. Full A–D spec stands above for a gateway-up session.
