...
### Session 2026-05-30 (cont.) — Wave 1 resolver finished + Wave 2 started

- [x] **Effective-model resolver — NOW SHIPPED** (supersedes the deferral above). Correction to the findings: the gateway `/messages` handler ALREADY reads `body.modelOverride` → `spawn --model` (`services/claude-gateway/src/index.ts:96/264`) — the same mechanism the chat per-turn override uses — so it was a CLIENT-ONLY change, no gateway redeploy.
  - `lib/ai/dispatch.ts` `resolveEffectiveModel` (the model-agnostic-chat M2 seam) + pure `pickModel`/`familyForModel` (`__tests__/effective-model.test.ts`, 17 assertions).
  - Dispatch route resolves the effective model → uses it for codex routing AND forwards `modelOverride` through the local `dispatchToOpenClaw`.
  - Verification ceiling: unit + typecheck + the proven gateway mechanism. The final live-dispatch assertion (`gateway_turns.model`) is the operator's confirmation — a dev+playwright session won't dispatch a real job to the prod gateway.
- [x] **Learning — difficulty-aware interleaving (Scope B)** — SHIPPED + verified E2E. `lib/learning/interleave.ts` (pure: track+moc buckets, weakest-first FSRS, no back-to-back same atom; `__tests__/interleave.test.ts`, 8 assertions). **Live-verified:** `GET /api/learn/session?size=10` → 200 with 10 real cards from the operator's deck (authenticated in-page fetch).
- [x] **Akashic Thread C — Graphify ADR** — `docs/adr/010-graphify-obsidian-dual-store.md` (decline + rationale) + indexed.
- [x] **Durable Chat (sub-plan 3) — SHIPPED + verified (Phases A–D).**
  - **A+B (core)** — `inflight_*` columns (migration 099), `setInflightTurn`/`claimInflightTurn`, `reconcile-inflight.ts` + `/api/cron/chat-turn-drain` + enqueue self-kick. **Verified E2E on Haiku** (`tests/playwright/durable-chat.spec.ts`): a turn's reply lands in `chat_messages` with the browser NEVER polling — drained server-side by the reconciler. `1 passed (1.4m)`.
  - **C (re-attach)** — `getInflightTurn` + messages route exposes `inflight_job_id`/`inflight_started_at`; `PlatformChat` session-load effect resumes `pollUntilDone` (guarded on `!busy` so it never double-polls an active send; `claimInflightTurn` guards the persist race). **Verified E2E:** endpoint exposes the live jobId while running, clears after drain.
  - **D (error inbox + notify)** — `chat-turn` NotificationCategory (toggleable in `/settings` Alerts); reconciler fires `notifyOperator(...,'chat-turn',...)` on a server-side drain (operator away); `markTurnError` stamps `metadata.turn_error` on the user message on enqueue-fail (survives reload, renders inline in `MessageBubble`); `loadChatTurnAlerts` surfaces crashed/errored turns in `/inbox` + the dashboard `FleetApprovalInbox`. tsc + retry-storm + cron-route + codeql + provider-agnostic all green.
  - Cheap-model testing: every chat turn in the spec runs on `claude-haiku-4-5` via `modelOverride` (`TEST_CHAT_MODEL`).
- [ ] **Learning concept curriculum (Scope A)** — interleave (B) shipped; concept CardKind + curriculum content + render (A1–A7) remain (need migration 098 applied to seed concept cards before E2E render).
- [ ] **Akashic Thread A (structural ecosystem graph)** — not started (migration + indexer cron + real graph builder; full verification is migration/seed-gated).

### Session 2026-05-30 (cont.3) — durable chat finished + settings parity (status reconciliation)

> The checklist above drifted from reality across sessions. Verified-against-disk status as of this session:

- [x] **Durable Chat (sub-plan 3) — SHIPPED + verified, Phases A–D** ([#448](https://github.com/pinnacleadvisors/nexus/pull/448)). A+B core verified E2E on Haiku (reconciler drains a detached turn into `chat_messages`, browser never polls); C re-attach (messages route exposes `inflight_job_id`, `PlatformChat` resumes the poll); D notify+inbox (`chat-turn` category, `markTurnError`, `loadChatTurnAlerts`). Supersedes the "NOT shipped" line above.
- [x] **Learning concept curriculum (Scope A) — ALREADY SHIPPED** (confirmed on disk: `lib/learning/concept-source.ts` + `concept-curriculum.ts` (6 lessons) + `concept-sync.ts`, wired into `sync-learning-cards` cron; `components/learn/ConceptCard.tsx`; `__tests__/concept-source.test.ts` 12/12 green). Migration 098 applied. Supersedes the "remain" line above.
- [x] **Settings UX parity (sub-plan 6) — Group A + C SHIPPED + verified** (this PR). New `GET /api/models/providers/available` (canonical enabled-provider set via `detectAvailableProviders`, respects the disabled toggle); `lib/hooks/useRecommendAll` (extracted 3-worker sweep); `SkillsListHeader` (Recommend-all); SkillCard manual model `<select>` sourced from the endpoint. **Verified E2E** (`tests/playwright/settings-skills-parity.spec.ts`): endpoint returns providers incl. `anthropic`, Recommend-all button + skill dropdowns render. Group B (Create intent cards) is the follow-up PR.
- [x] **Providers-Routing PR7 (relabel) — SHIPPED** (this PR). ActiveProviderSwitch → "Default LLM provider"; copy clarifies it's the context-free fallback default and per-agent/per-skill models take precedence (now that the resolver ships).
- [ ] **Still open:** Settings UX parity Group B (Create cards); Providers PR8 (auto_route smart-routing, opt-in); Akashic Thread A mol_edge persistence (the `/graph` render shipped in #447, the structural-edge migration + indexer cron did not); Gamify-LifeOS (Accomplishments + Health); Local-First (deferred on risk); Learning Scope C (distill-bridge).

### Session 2026-05-30 (cont.6) — Group B + Accomplishments shipped + multi-PR plan-file hygiene

> This is the ONE PR of the batch that touches `task_plan` — feature PRs #450/#451 were made code-only to stop the parallel-append merge conflict (new AGENTS.md rule "Parallel-PR shared-file discipline").

- [x] **Settings UX parity (sub-plan 6) — Group B SHIPPED + verified** ([#450](https://github.com/pinnacleadvisors/nexus/pull/450)). Connectors-style "Create" intent cards on the Agents + Skills tabs (`DescribeIntentCard` + `POST /api/agents/describe` → agent-generator brief + `POST /api/skills/describe` → skill-trainer brief, both retry-storm-safe). Verified E2E (`settings-create-cards.spec.ts`): both cards render; describe endpoint reachable + fail-soft. **Settings UX parity (sub-plan 6) is now fully shipped (A+B+C).**
- [x] **Gamify Accomplishments — read-path v1 SHIPPED + verified** ([#451](https://github.com/pinnacleadvisors/nexus/pull/451)). `/accomplishments` "Wins" board: `lib/accomplishments/{catalog,revenue,evaluate}.ts` (real-revenue tiers via the dashboard's `experiment_metrics`/`payloadUsd` extractor, all-time) + `GET /api/accomplishments` (pure-read) + mobile-first page + sidebar nav. Verified E2E at desktop (chromium) AND mobile (android 393px). Deferred (migration-gated): unlock-persistence + `milestone` notify + recompute cron; Health LifeOS (`life_domains`).
- [x] **AGENTS.md — "Parallel-PR shared-file discipline" rule added** (pre-commit checklist) so future multi-PR sessions keep feature PRs code-only + carry plan/Progress updates in one PR.
- [ ] **Still open (all migration-gated except Learning Scope C):** Akashic Thread A `mol_edge` persistence; Gamify unlock-persistence + Health LifeOS; Providers PR8 (`auto_route`); Learning Scope C (distill-bridge, no migration); Local-First (deferred on risk).
