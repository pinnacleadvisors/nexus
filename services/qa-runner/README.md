...
| `{ ok: false, error: 'unauthorized', reason: 'no_authorization_header' }` | The caller (cron-job.org job, manual curl) sent no `Authorization` header at all | cron-job.org dashboard → edit the `Nexus: cron-post-deploy-smoke` job → set `Authorization: Bearer $CRON_SECRET` in request headers. For manual curl, add `-H "Authorization: Bearer $CRON_SECRET"`. |
| `{ ok: false, error: 'unauthorized', reason: 'cron_secret_unset_and_bot_token_invalid' }` | `CRON_SECRET` env var not set in the Nexus deployment AND the caller didn't send a valid `BOT_API_TOKEN` either | `doppler secrets set CRON_SECRET=<long-random-string> --config prd`. Then re-run `doppler run -- node scripts/migrate-crons-to-cronjob-org.mjs --apply` so the cron-job.org headers match. |
| `{ ok: false, error: 'unauthorized', reason: 'cron_secret_set_but_header_mismatch' }` | `CRON_SECRET` is set in Doppler but the caller's `Authorization` header carries a different value (rotated since the cron-job.org job was created) | Either rotate cron-job.org's stored header to match the new `CRON_SECRET`, OR rotate `CRON_SECRET` back and redeploy. The migration script (`--apply`) updates existing jobs in place. |
