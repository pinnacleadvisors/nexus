/**
 * Single-worker FIFO queue. The ChatGPT Pro plan is one identity, so we
 * serialise concurrent dispatches into a single codex CLI process at a time.
 * When the queue is full we reject with QueueFullError so callers fail fast
 * rather than holding sockets open.
 */
