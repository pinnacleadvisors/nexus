#!/usr/bin/env bash
# Smoke-test the Claude Code gateway end-to-end.
#
# Hits GET /health (unauth), then a signed POST /api/sessions/smoke/messages
# that prompts the spawned claude CLI to reply "pong". Exits non-zero on any
...
#
# Usage:
#   BEARER=<hex> HOST=https://claude-gw.<your-domain> ./smoke.sh
...
if [ -z "$HOST" ]; then
  printf 'Gateway host (e.g. https://claude-gw.example.com): '
  read -r HOST
fi
if [ -z "$BEARER" ]; then
  printf 'CLAUDE_GATEWAY_BEARER: '
...
case "$logged_in" in
  *true*)  echo "✓ /health OK and Claude is logged in" ;;
  *false*) echo "⚠ /health OK but claude CLI is NOT logged in. Run: docker exec -it \$CT claude login" ;;
...
case "$signed_code" in
  ...
  401)
    ...
    case "$reason" in
      bad-bearer)
        echo "  Cause: BEARER doesn't match server's CLAUDE_GATEWAY_BEARER." >&2
    ...
    ;;
  502)
    echo "✘ 502 — gateway received the request but the claude CLI failed." >&2
    echo "  Common cause: claude CLI not logged in. Run: docker exec -it \$CT claude login" >&2
    exit 1
    ;;
  503)
