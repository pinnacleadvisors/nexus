...
- [x] **Lint warnings (orphan portion)** — fixed 2026-05-27. `memory/molecular/mocs/agent-framework-survey.md` body now lists each of the 11 atoms via `[[wikilinks]]` grouped into "Patterns absorbed" / "Anti-patterns rejected" / "Trial" / "Nexus-originated patterns". `cli.mjs lint` now reports `orphanAtoms: 1` (only the unrelated `learn-page-cron-runs-at-05-utc-with-manual-button`), down from 11. `sourcelessAtoms` and `emptyEntities` are separate lint categories (each atom needs `locators:` frontmatter; `higgsfield` + `openswarm` entity files are empty) — those are independent follow-ups not covered by the "wikilink atoms in MOC body" quick fix.
...
- [x] **Skill-promote UI integration** — shipped 2026-05-27 in [`components/settings/SkillsList.tsx`](components/settings/SkillsList.tsx). Promote button appears next to the `draft` pill on every draft-status skill card in `/settings → Skills`. confirm() dialog warns about the commit+redeploy persistence step. Backend route was already shipped in the original Task 17. Landed in Settings UI rather than the Board ReviewModal because (a) it sits next to the per-skill model-recommend affordance from PR #388, and (b) the Board surface is for run-review work, not skill-management work — Settings is the canonical home for skill lifecycle.
